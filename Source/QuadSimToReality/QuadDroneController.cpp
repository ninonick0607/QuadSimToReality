...
static TArray<FVector> simplePath()
{
    const int step = 1000;
...
UQuadDroneController::UQuadDroneController(const FObjectInitializer &ObjectInitializer)
    : dronePawn(nullptr), Thrusts({0, 0, 0, 0}), desiredYaw(0.f), bDesiredYawInitialized(false), desiredAltitude(0.0f), bDesiredAltitudeInitialized(false), currentFlightMode(FlightMode::None), currentNav(nullptr), curPos(0), AutoWaypointHUD(nullptr), VelocityHUD(nullptr), JoyStickHUD(nullptr), ManualThrustHUD(nullptr), desiredNewVelocity(FVector::ZeroVector), maxVelocity(250.0f), maxAngle(15.f), initialTakeoff(true), altitudeReached(false), Debug_DrawDroneCollisionSphere(true), Debug_DrawDroneWaypoint(true), thrustInput(0.0f), yawInput(0.0f), pitchInput(0.0f), rollInput(0.0f), hoverThrust(0.0f), bHoverThrustInitialized(false), ZMQSocket(nullptr), CommandSocket(nullptr), SceneCaptureComponent(nullptr), RenderTarget(nullptr), UpdateInterval(0.0001f), TimeSinceLastUpdate(0.0f), bIsActive(true), DroneID(TEXT("DefaultDrone"))
{
    ...
    AddNavPlan("Spiral Path", spiralWaypoints());
    SetNavPlan("Spiral Path");
...
{
    if (!currentNav || curPos >= currentNav->waypoints.Num())
        return;
