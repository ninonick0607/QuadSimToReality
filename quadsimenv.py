...
class QuadSimEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Stability rewards - INCREASED PENALTIES
        if hasattr(self, 'prev_velocity'):
            velocity_change = abs(z_velocity - self.prev_velocity)
            stability_penalty = -velocity_change * 0.1  # Increased from 0.1
            reward += stability_penalty
            
            # Much stronger overshoot penalty
            if self.prev_velocity * z_velocity < 0:  # Direction changed
                overshoot_penalty = -abs(z_velocity) * 1.0  # Increased from 0.2
                reward += overshoot_penalty
        
        self.prev_velocity = z_velocity

        # Energy efficiency - INCREASED PENALTY
        energy_penalty = -abs(z_velocity) * 0.05  # Increased from 0.01
